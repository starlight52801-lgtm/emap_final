...
csvFileName = 'predictions/output/dt_reprod_gsr/results_DT.csv'
...
for filename in all_files:
    ...
    if match is None:
        print(f"Skipping file without participant ID: {basename}")
        continue
    ...
    participant_to_files[participant_id].append(filename)

available_participants = sorted(participant_to_files.keys())
print("Available participants:", len(available_participants))

if len(available_participants) < 3:
    raise ValueError(
        "At least 3 participants are required for train/val/test splitting."
    )

# Create up to 5 folds, but never more folds than participants
n_folds = min(5, len(available_participants))
participant_folds = np.array_split(available_participants, n_folds)

list_of_lists = []
for fold_idx, fold_participants in enumerate(participant_folds):
    fold_files = []

    for participant_id in fold_participants:
        for filename in participant_to_files[participant_id]:
            reader = pd.read_csv(filename, encoding='ISO-8859-1', header=0)
            fold_files.append(reader)

    if len(fold_files) > 0:
        list_of_lists.append(fold_files)

print("Number of non-empty folds:", len(list_of_lists))
for idx, fold in enumerate(list_of_lists):
    print(f"Fold {idx}: {len(fold)} files")
...
for i in range(len(list_of_lists)):
    ...
    reg = DecisionTreeRegressor(max_depth = max_depth_DT, random_state=0).fit(X_train, y_train)
    ...
    def f_per_particle(m, alpha):
        ...
        reg = DecisionTreeRegressor(max_depth = max_depth_DT, random_state=0).fit(X_subset, y_train)
    ...
    options = {'c1': 0.5, 'c2': 0.3, 'w':0.7, 'k': 10, 'p':2}
    ...
    optimizer = ps.single.GlobalBestPSO(n_particles=10, dimensions=dimensions, options=options, bounds = values_bound)
    
    # Perform optimization
    cost, pos = optimizer.optimize(f, iters=20) #1000
    ...
    reg = DecisionTreeRegressor(max_depth = max_depth_DT, random_state=0).fit(X_selected_features, y_train)
    ...
    finalFeatures = "predictions/output/dt_reprod_gsr/FS_DT_pos" + str(iterate) + ".csv"
